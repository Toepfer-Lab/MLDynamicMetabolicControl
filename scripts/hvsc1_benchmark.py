"""
Benchmark: LP solver (cooperative_tradeoff) vs trained surrogate, for the
27-taxon hvsc1 community. Mirrors scripts/mcsm_benchmark.py exactly (same
structure/fields in the saved .npz) so scripts/complexity_comparison.py and
scripts/hvsc1_plot_benchmark.py (mirroring mcsm_plot_benchmark.py) can
...
Results saved to results/hvsc1_benchmark.npz.

The LP section requires CPLEX -- run this script via sbatch
(scripts/hvsc1_benchmark.sh).
"""
...
CHECKPOINT  = REPO_ROOT / "trained_models" / "hvsc1_community_input-27_output-27_hidden-128.pt"
...
FRACTION = 0.5
MIN_ABUND = 1e-8
SEED     = 99    # different from training/reserved-test seed ranges (1000+, 2000+, 9000+)
...
def apply_medium_and_unlock(comm, default_medium):
    ...
    hvsc1 has no external medium CSV (unlike mcsm) -- its own default exchange
    bounds are the medium, same convention as hvsc1_simulate.py/hvsc1_generate_data.py."""
    comm.medium = default_medium
    for rxn in comm.internal_exchanges:
        rxn.bounds = (-1000.0, 1000.0)


def lp_solve(comm, x, taxa_ids, default_medium):
...
# Capture default medium BEFORE any set_abundance call (same convention as
# hvsc1_simulate.py / hvsc1_generate_data.py)
...
for i, x0 in enumerate(starts_lp):
    ...
    for step in range(N_STEPS):
        mu, status, elapsed = lp_solve(comm, x, taxa_ids, default_medium)
...
out_path = RESULTS_DIR / "hvsc1_benchmark.npz"
np.savez_compressed(
    out_path,
    # LP
    n_lp              = N_LP,
